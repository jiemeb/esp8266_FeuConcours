...
EasyNTPClient::EasyNTPClient (UDP &udp, const char* serverPool, int offset, int interval) {
  this->mUdp = &udp;
  this->mServerPool = serverPool;
  this->mOffset = offset;
...
int EasyNTPClient::getUpdateInterval() {
  return this->mUpdateInterval;
}

void EasyNTPClient::setUpdateInterval (int offset) {
  this->mUpdateInterval = offset;
}
